...
void InputManager::controllerUpdate()
{
	...
	for(int a = 0; a < GLFW_JOYSTICK_LAST; a++)
		if(glfwJoystickPresent(a))
		{
			...
			m_controllers[a].triggers[0] = (m_controllers[a].triggers[0] + 1)*.5;//triggers have values from 0 -> 1
			m_controllers[a].triggers[1] = (m_controllers[a].triggers[1] + 1)*.5;//triggers have values from 0 -> 1
